    ...
    if(mysqli_num_rows($result)<>0) {
        ...
        while($rows=mysqli_fetch_assoc($result)) {
            ...
            if ($rows['tenLKM'] === 'Khuyến mãi %')
            {
                ...
                $giaSauGiam=$rows['gtDonHang']-($rows['gtDonHang']*$rows['giamGia'])/100;
            }
            elseif ($rows['tenLKM'] === 'Khuyến mãi Giá')
            {
                $noidungBang .= "<td>" . number_format($rows['giamGia'], 0, ",", ".") . "đ</td>";
                $giaSauGiam=$rows['gtDonHang']-$rows['giamGia'];
            }
            $noidungBang .= "<td>" . number_format($rows['gtDonHang'], 0, ",", ".") . "đ</td>";
            $noidungBang .= "<td>" . number_format($giaSauGiam, 0, ",", ".") . "đ</td>";
...
<main>
    ...
    <table border='1'>
        <tr>
            ...
            <th>Giá trị đơn hàng sau khi giảm</th>
            <th>Ngày bắt đầu</th>
            <th>Ngày kết thúc</th>
            <th>Thao tác</th>
        </tr>
